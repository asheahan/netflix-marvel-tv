...
for season in range(1, 2):
    for file in glob.glob('data/iron-fist/rottentomatoes/critic-reviews-season-' \
        + str(season) + '-*'):
        with open(file, 'r') as handle:
            soup = BeautifulSoup(handle, "html.parser")
            for row in soup.select("div.review_table_row"):
                review = {}
                review['show'] = 'Iron Fist'
    ...
    for file in glob.glob('data/iron-fist/rottentomatoes/audience-reviews-season-' \
        + str(season) + '-*'):
        with open(file, 'r') as handle:
            soup = BeautifulSoup(handle, "html.parser")
            for row in soup.select("div.review_table_row"):
...
with codecs.open('data/iron-fist/critic-reviews.json', 'w', 'utf-8') as handle:
    json.dump(critic_reviews, handle)
with codecs.open('data/iron-fist/audience-reviews.json', 'w', 'utf-8') as handle:
