...
for season in range(1, 2):
    for file in glob.glob('data/jessica-jones/rottentomatoes/critic-reviews-season-' \
        + str(season) + '-*'):
        with open(file, 'r') as handle:
            soup = BeautifulSoup(handle, "html.parser")
            for row in soup.select("div.review_table_row"):
                review = {}
                review['show'] = 'Jessica Jones'
    ...
    for file in glob.glob('data/jessica-jones/rottentomatoes/audience-reviews-season-' \
        + str(season) + '-*'):
        with open(file, 'r') as handle:
            soup = BeautifulSoup(handle, "html.parser")
            for row in soup.select("div.review_table_row"):
...
with codecs.open('data/jessica-jones/critic-reviews.json', 'w', 'utf-8') as handle:
    json.dump(critic_reviews, handle)
with codecs.open('data/jessica-jones/audience-reviews.json', 'w', 'utf-8') as handle:
